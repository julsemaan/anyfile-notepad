...
Preferences.prototype.get_from_drive = function(){
  ...
  if(self.cache.get_cache(User.get_session_user_id()+"file_id")) {
    return self.build_from_file_id(self.cache.get_cache(User.get_session_user_id()+"file_id"));
  }
  ...
  application.controllers.google_oauth.execute_request(request, function(response){
    ...
    for(var i=0; i<files.length;i++){
      if(files[i].title == "preferences"){
        ...
        self.cache.cache(User.get_session_user_id()+"file_id", files[i].id);
        return
      }
    }
    self.create_initial()
