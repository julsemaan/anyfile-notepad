...
Captured on 2026-09-07 from source commit `46c8828884a89728b243ec86202f2156e218dbda` on `chore/client-dependency-baseline`.
...
- `package.json` declares five npm dependencies.
- `package-lock.json` is lockfile version 1 with 252 unique package names and 292 physical package entries, including nested overrides. It is the complete committed npm name and version list. The Dockerfile does not copy it before running `npm install`, so the installed image is not proven to match it.
- `bower.json` declares four Bower dependencies. There is no committed Bower lockfile.
- `client/base/Dockerfile` uses the mutable `node:16-buster` image and installs unpinned apt and CPAN inputs. The exact Node, npm, system package, and Perl module versions are not recorded here.
...
| Package | Declaration | Lock resolution | Exposure | Source and advisory evidence from 2026-09-07 |
|---|---|---:|---|---|
| `bower` | `>0.0.0` | 1.8.8 | Build and install only | npm registry; not listed by this audit. Bower-managed browser code is outside npm audit. |
| `dropbox` | `<3.0.0` | 2.5.13 | Browser code: `Dropbox-sdk.min.js` is appended to `application.js` | npm registry; npm audit reported a high direct finding through `superagent-proxy`. |
| `minify` | `2.1.8` | 2.1.8 | Build only | npm registry; npm audit reported a high direct finding through `clean-css` and `html-minifier`. |
| `node-sass` | `>0.0.0` | 4.14.1 | Build only | npm registry; npm audit reported a high direct finding with vulnerable transitive build inputs. |
| `tether-shepherd` | `^1.8.1` | 1.8.1 | Browser code: Shepherd and Tether CSS/JS are appended to `application.js` and CSS | npm registry; not listed by this audit. Runtime/browser exposure was not tested. |

The lockfile's top-level transitive closures contain these counts. They overlap, so the counts must not be added together.

| Direct root | Lock entries in its top-level `requires` closure | Main role |
|---|---:|---|
| `bower` | 1 | Bower installer |
| `dropbox` | 75 | Dropbox SDK and request stack |
| `minify` | 27 | CSS and JavaScript minification |
| `node-sass` | 165 | Sass compiler and native build stack |
| `tether-shepherd` | 2 | Tether and Shepherd runtime |

For the exact version, integrity, and nested package data, use the committed [`package-lock.json`](package-lock.json). The lock includes multiple versions of some transitive names, including `debug`, `ms`, `source-map`, `uglify-js`, and `readable-stream`.

The dated audit was run against a temporary copy of the two npm manifests with `npm audit --package-lock-only --ignore-scripts` using npm 11.12.1. It returned exit status 1 with 42 package findings: 7 critical, 27 high, 6 moderate, and 2 low. This is package-manager evidence, not a browser exploitability assessment. It does not cover Bower, vendored files, the nested Ace tree, or remote scripts.

## Other build inputs

| Input | Source | Use |
|---|---|---|
| Node/npm | `client/base/Dockerfile`: `node:16-buster` | npm install, the client build, and the nested Ace build |
| apt tools | `perl`, `gcc`, `curl`, `make`, `inotify-tools` | Template rendering, native npm builds, resource downloads, and webdev watching |
...
Versions and digests for these inputs are not available from the committed source.
...
|---|---|---|---|
| `ace-anyfile-notepad` | `anyfile-notepad-v1.3.3` | `afn-app.sh` runs a second `npm install` and `make afn-dist` inside the component, then copies `afn-dist/*` to `/ace.js/`. | Custom fork is identified by the tag and the [fork URL](https://github.com/julsemaan/ace/tree/anyfile-notepad-v1.3.3). The resolved commit and nested package tree are unavailable. |
...
|---|---|---|
| `assets/js/libs/handlebars.js` | Handlebars 4.0.5 | Runtime compilation for initial templates, partials, popups, and flash messages. App and site bundles. |
...
| `assets/css/libs/bundled-material.min.css.scss` | Version and source commit not recorded; includes Material Design Icons and Bootstrap Material rules | Imported into application and page CSS. |
...
- `application.js` contains the Dropbox SDK, Bower jQuery, jQuery UI, Bootstrap, Shepherd/Tether, file-tree code, cookie code, and all `assets/js/*.js` files.
- `application.css` contains Bootstrap, Shepherd, file-tree CSS, and compiled editor CSS. `pages.css` contains Bootstrap and the shared material/page styles.
...
| `https://code.jquery.com/jquery-1.11.3.min.js` | Print window creation | `assets/js/Controller/Editor.js` |
