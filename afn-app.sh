...
function pages() {
  ...
  for page in home faq news help_translate privacy_policy payment-success payment-failure terms-of-service email-cancel; do
...
function start_server() {
  ...
  cd webserver/
  find . -maxdepth 1 -name '*.go' ! -name '*_test.go' | xargs go run 2>&1 &
  cd -
  WEB_PID=$!
