...
.PHONY: check-go
check-go:
	@unformatted="$$(find api webserver utils -type f -name '*.go' -exec gofmt -l {} +)"; \
	...
	cd webserver && go vet ./...
	cd api && go vet -mod=mod ./...
	cd utils && go vet ./...
	cd webserver && go test -race -cover ./...
	cd api && go test -race -cover -mod=mod ./...
	cd utils && go test -race -cover ./...
